...
num_samples_df <- merge(data.frame(rep=1:3), data.frame(nonbrain = c(10,20,30,40)) )
...
results <- foreach(row_id=1:nrow(num_samples_df), .errorhandling = "pass",
   ...
   for(measure in specificity_measures$func_names ){
     ...
     specificity_flat_bal <- specificity_func(mat_bal, flat_bal)
     specificity_weighted_bal <- specificity_func(mat_bal, weights_bal)
     
     specificity_flat_unbal <- specificity_func(mat_unbal, flat_unbal)
     specificity_weighted_unbal <- specificity_func(mat_unbal, weights_unbal)
