...
if(TRUE){
  ...
  col_funs <- list()
  col_funs[["Zscore"]] <-colorRamp2(c(min(c(-2, min(spec_mats$delta_Zscore, na.rm = T)) ),
                                      -1.99, -1, 0, 1, 1.99,
  ...
  for(measure in specificity_measures$func_names[which(specificity_measures$out_type=="vector")]){
    col_funs[[measure]] <-colorRamp2(c(-1,-0.25, 0 , 0.25, 1),
  ...
  draw(heatmaps, padding = unit(c(1, 0.1, 0.1, 0.1), "in"))
