...
df <- data.frame(n=numeric(), rep=numeric(), cut_point=numeric(),
                 ...
                 gene_count=numeric(), gene_overlap_count=numeric())
  ...
  df_temp <- foreach(i=1:length(robustness_test_results[[el]]), .errorhandling = "pass", .combine = rbind) %dopar% {
    ...
    for(weighted_or_flat in c("weighted","flat")){
      ...
      temp_df <- data.frame(n=rep(n, ncols*ncuts), rep=rep(rep, ncols*ncuts), cut_point=numeric(length=ncols*ncuts),
                   ...
                   gene_count=numeric(length=ncols*ncuts), gene_overlap_count=numeric(length=ncols*ncuts) )
      for(s in 1:ncol(temp_mat) ){  ## add a row to df for each sample   
        for(cut_index in 1:length(cuts)){
          ...
          if(cuts[cut_index]<0){
            ...
            temp_df[ind,]$gene_overlap_count <-  length( which( baseline_mat[,s] < cuts[cut_index] & temp_mat[,s] < cuts[cut_index]  ))
          } else {
            temp_df[ind,]$jaccard <- (  length( which( baseline_mat[,s] > cuts[cut_index] & temp_mat[,s] > cuts[cut_index]  )) /
                                        length( which( baseline_mat[,s] > cuts[cut_index] | temp_mat[,s] > cuts[cut_index]  )) )
            temp_df[ind,]$gene_count <-  length( which(temp_mat[,s] > cuts[cut_index]  ) )
...
df_summary$count_mean <- aggregate(df$gene_count, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,samp=df$samp), FUN=function(x) mean(x,na.rm = TRUE) )$x 
df_summary$count_sd <- aggregate(df$gene_count, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,samp=df$samp), FUN=function(x) sd(x,na.rm = TRUE) )$x 
df_summary$overlap_count_mean <- aggregate(df$gene_overlap_count, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,samp=df$samp), FUN=function(x) mean(x,na.rm = TRUE) )$x 
df_summary$overlap_count_sd <- aggregate(df$gene_overlap_count, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,samp=df$samp), FUN=function(x) sd(x,na.rm = TRUE) )$x 
...
for(samp in unique(df_summary$samp)){
  gg<-ggplot(df_summary[which(df_summary$samp==samp),], aes(x=cut_point, y=jaccard_mean, color=n, group=n))+
    ...
    facet_wrap( ~ weighted)
 plot(gg)
 gg<-ggplot(df_summary[which(df_summary$samp==samp),], aes(x=cut_point, y=log10(count_mean), color=n, group=n))+
   ggtitle(paste(samp))+
   geom_line()+
   facet_wrap(~ weighted)
 plot(gg)
...
gg
# ggsave( paste( figures_dir, "/", date, "real_partition_jaccard_index_P1_v_P2_flat_v_weighted_v01.png",sep = "") , gg, device = "png", dpi=300, width = 8, height = 8)
