...
#"NEUROD1","NEUROD2" ,"NEUROD4", "NEUROD6",
specific_genes <- c("SOX2", "MAP2", "MBP", "OLIG1","GFAP" , "PRSS1", "MYH6", "MYH7")
...
for(measure in unique(str_split_fixed(names(spec_mats), "_", 2)[,2] )){
  if(measure=="Zscore"){
    col_funs[[measure]] <-colorRamp2(c(min(c(min(spec_mats$weighted_Zscore[spec_genes_w_ids$Name,], na.rm = TRUE ),
                                     ...
                                     c("darkblue", "blue", "#7777ff" ,"white", "#ff7777",  "red","red4"))  
  } else {
...
for(measure in unique(str_split_fixed(names(spec_mats), "_", 2)[,2] )){
  ...
  if(measure=="Zscore"){
    split <- rep(1:length(specific_genes), each=2)
    ra <- rowAnnotation(foo = anno_block(labels = specific_genes))
