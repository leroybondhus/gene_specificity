...
```{r}
##balanced v unbalanced correlation
...
which_brain <- grep("Brain", colnames(exp_mat) )
balanced_v_unbalanced_cor <- foreach(i=1:length(which_brain), .combine=rbind) %dopar% {
  which <- c(grep("Brain", colnames(exp_mat), invert = TRUE ), which_brain[i])
  ...
  for(measure in specificity_measures$func_names){
    for(f_or_w in c("flat","weighted")){
      ...
      if(f_or_w == "flat"){weights[] <- 1}
      unbalanced <- specificity_func(exp_mat, weights)
      
      if(measure=="Zscore"){
        temp_cor <- cor(as.vector(balanced),as.vector(unbalanced[,colnames(balanced)]), use="pairwise.complete")
      } else {
        temp_cor <- cor(as.vector(balanced),as.vector(unbalanced), use="pairwise.complete")
      }
      balanced_v_unbalanced_cor <- rbind(balanced_v_unbalanced_cor,
                                         data.frame(
                                                measure = measure,
                                                weighted = ifelse(f_or_w=="flat",F,T),
                                                rep = i,
                                                cor = temp_cor))
    }
  }
  balanced_v_unbalanced_cor
}
write.csv(balanced_v_unbalanced_cor, file = paste(figures_dir,"/balanced_v_unbalanced_cor_table.csv",sep = ""))

for(m in unique(balanced_v_unbalanced_cor$measure)){
  flat_cor <- balanced_v_unbalanced_cor[which(balanced_v_unbalanced_cor$measure==m & 
                                           balanced_v_unbalanced_cor$weighted),]
  weight_cor <- balanced_v_unbalanced_cor[which(balanced_v_unbalanced_cor$measure==m & 
                                           !balanced_v_unbalanced_cor$weighted),]
  print(m)
...
grid.arrange(grobs=grobl_main, ncol=4, widths=c(5,3,3,3))
g <-arrangeGrob(grobs=grobl_main, ncol=4, widths=c(4,4,4,4))
filename = paste(figures_dir, "/main_one_v_all.png", sep="")
ggsave(filename, g, device = "png", width = 10, height = 2.8, dpi = 250)

grid.arrange(grobs=grobl_supp, nrow=4, heights=c(5,3,3,3))
g <-arrangeGrob(grobs=grobl_supp, nrow=4, heights=c(4,4,4,4))
filename = paste(figures_dir, "/supp_one_v_all.png", sep="")
ggsave(filename, g, device = "png", width = 4, height = 10, dpi = 250)

```


```{r plot difference between 1 and all brain samples included for weighted and flat, fig.width = 8, fig.height=8}
grid.arrange(grobs=grobl_supp, nrow=4, heights=c(5,3,3,3))
```
...
## plot on one v all plot
 ...
 #                mart = ensembl, useCache = F)
load("genes_w_go.RData")

allOE_genes<-str_split_fixed(rownames(exp_mat),"[.]",2)[,1]
ggl <- list()
for(f_or_w in c("flat","weighted")){
  for(measure in specificity_measures$func_names){
    for(slice_func in c("max", "min")){
      spec_delta_cut <- ifelse(measure=="Zscore",1,0.1)
      ...
      if(slice_func=="min"){
        temp_df_sub <- slice_min(temp_df, order_by=delta, n=nrow(temp_df)/100 )
      }
      
      temp_df_sub <- temp_df_sub[which(abs(temp_df_sub$delta) > spec_delta_cut),]
      ...
      n=10
      filename <- paste(figures_dir,"/",measure,"_",f_or_w,"_", slice_func,
                        "v3_1percent_deltas_go.png",sep = "")
      p1 <- ggl[[measure]]
      ...
      png(filename, width = 18, height = 5.5, units = "in", res=200 )
...
```{r measure sample similarity, fig.width = 8, fig.height=5}
...
sim_tree %>% dendextend::set("nodes_pch",19) %>% dendextend::set("nodes_cex", 2*sqrt(get_nodes_attr(sim_tree,"weight"))) %>% plot
dev.off()
...
if(TRUE){  ## NOTE! This chunk can require a lot of memory and time depending if running sequential. If you want to look at robustness test results it is recommended you have enabled parallel computing (e.g. check getDoParWorkers > 3 or 4 (preferably more))
  ...
  if(random_partition){
    ...
    notP1 <- as.matrix(exp_mat[,unique(as.vector(permutation_matrix)) ])
    P1 <- as.matrix(exp_mat[,colnames(exp_mat)[ !is.element(colnames(exp_mat),colnames(notP1))]])
  }
...
for(measure in c("Zscore", "notZscore")){
  ...
  gg <- ggplot(df_2c_summary_by_partition[which,], aes(x=n, y=mean_var)) +
    ...
    geom_ribbon(aes( group=weight_or_flat, ymin=y_min, ymax=y_max, fill=weight_or_flat),alpha=0.3)+
...
filename <- paste(figures_dir,"/",date,"_variance_v01.png",sep = "")
...
if(random_partition){
  ...
  filename <- paste(figures_dir,"/",date,"_variance_random_v02.png",sep = "")
...
filename=paste(figures_dir,"/ttest_output.txt",sep="")
sink(file=filename)
...
g <- arrangeGrob(grobs =grobl, ncol=2, widths=c(5,4) )
filename <- paste(figures_dir,"/",date,"_jaccard_v01.png",sep = "")
...
if(random_partition){
  ...
  filename <- paste(figures_dir,"/",date,"_jaccard_random_v02.png",sep = "")
  ggsave(filename, g, device = "png", width = 7, height = 7, dpi = 300)
}
...
filename <- paste(figures_dir,"/",date,"_jaccard_gene_count_v01.png",sep = "")
...
filename=paste(figures_dir,"/ttest_jaccard_output.txt",sep="")
sink(file=filename)
...
for(i in 1:length(spec_mats)){
  if(grepl("delta|flat",names(spec_mats)[i])){next;}
  temp_list[[names(spec_mats)[i] ]] <- merge(spec_genes_w_ids, rbind( cbind(Name=rownames(spec_mats[[1]]),
                                                                            weighted_or_flat = "weighted",
                                                                            data.frame(specificity=signif(spec_mats[[i]],3) )),
                                                                      cbind(Name=rownames(spec_mats[[1]]),
                                                                            weighted_or_flat = "flat",
                                                                            data.frame(specificity=signif(spec_mats[[i+1]],3) ))
                                                                      ))
}

filename <- paste(figures_dir, "/specificity_scores_markers_Gtex.xlsx",sep = "")
...
specific_genes <- c( "OLIG1","OLIG2", "DRD1",
                     "PRSS1", "CTRB2", # "CTRB2",
                     "MYH6", "MYL4")
...
png(filename, width = 8, height = 7.5, units = "in", res=300 )
draw(heatmaps)
dev.off()
...
```{r get correlation stats}

balanced_v_unbalanced_cor_table

weighted_bal_v_unbal_cor_table <-
  balanced_v_unbalanced_cor_table[which(balanced_v_unbalanced_cor_table$weighted),]
colnames(weighted_bal_v_unbal_cor_table)[which(colnames(weighted_bal_v_unbal_cor_table)=="cor")] <- "w_cor"
weighted_bal_v_unbal_cor_table$weighted <- NULL
weighted_bal_v_unbal_cor_table$X <- NULL

flat_bal_v_unbal_cor_table <-
  balanced_v_unbalanced_cor_table[which(!balanced_v_unbalanced_cor_table$weighted),]
colnames(flat_bal_v_unbal_cor_table)[which(colnames(flat_bal_v_unbal_cor_table)=="cor")] <- "f_cor"
flat_bal_v_unbal_cor_table$weighted <- NULL
flat_bal_v_unbal_cor_table$X <- NULL

bal_v_unbal_cor_table <- merge(flat_bal_v_unbal_cor_table, weighted_bal_v_unbal_cor_table)
bal_v_unbal_cor_table$delta <- bal_v_unbal_cor_table$w_cor - bal_v_unbal_cor_table$f_cor

for(measure in unique(balanced_v_unbalanced_cor_table$measure)){
  ## delta here
  which <- which(bal_v_unbal_cor_table$measure==measure) 
  print(measure)
  print("delta")
  print(t.test(bal_v_unbal_cor_table$w_cor[which],bal_v_unbal_cor_table$f_cor[which], alternative = "greater"))
  #print("flat")
  #print(t.test(bal_v_unbal_cor_table$f_cor[which]))
}
