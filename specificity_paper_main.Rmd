...
```{r Supplemental figure go terms associated with difference in one v all brain samples}
...
sim_mat<- (similarity_func(exp_mat))
sim_mat2 <- calc_dot_product_similarity_matrix(exp_mat)
heatmap(sim_mat, scale = "none", margins=c(11,13))
...
calc_manhattan_matrix<- function(dat) {
  ...
  for(i in 1:ncol(dat)){
    for(j in 1:ncol(dat)) {
       manhatt[i,j]<-sum(abs(dat[,i]-dat[,j]))
    }
  }
  for(i in 1:nrow(manhatt)){
    manhatt[,i] <- manhatt[,i] / max(manhatt)
  }  
  return(manhatt)
}
manhat_matrix<- (calc_manhattan_matrix(exp_mat))
heatmap(manhat_matrix, scale = "none", margins=c(11,13))
