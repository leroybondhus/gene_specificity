...
for(el in names(robustness_test_results)){
  ...
  if(measure == "Zscore"){cuts <- seq(0,4,0.25)   ## set cutoffs
...
df_summary <-aggregate(df$jaccard, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,samp=df$samp), FUN=mean ) 
names(df_summary)[length(names(df_summary))] <- "jaccard_mean"
df_summary$jaccard_sd <- aggregate(df$jaccard, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,samp=df$samp), FUN=sd )$x 
...
ggplot(df, aes(x=cut_point, y=jaccard, color=n))+
  geom_smooth()+
  facet_wrap( ~ weighted)

ggplot(df, aes(x=cut_point, y=jaccard, color=n, group=n))+
  geom_smooth()+
  facet_wrap( ~ weighted)

temp <- df[which(df$weighted & is.element(df$samp, unique(df$samp)[1]) & df$n == 9 ),]
ggplot(temp, aes(x=cut_point, y=jaccard, color=n ,group=n))+
  geom_line()+
facet_wrap( ~ samp)
```
