...
for(is_main in c(T)){ #,F)){
  for(measure in specificity_measures$func_names){
    temp <- data.frame(cor_f = cor(df$all[which(df$f_or_w=="flat" & df$measure==measure)],
                                   df$one[which(df$f_or_w=="flat" & df$measure==measure)], use="complete.obs"),
                       cor_w = cor(df$all[which(df$f_or_w=="weighted" & df$measure==measure)],
                                   df$one[which(df$f_or_w=="weighted" & df$measure==measure)], use="complete.obs"),
                       min = min(c(df$all[which(df$measure==measure)],df$one[which(df$measure==measure)]), na.rm = T),
                       max = max(c(df$all[which(df$measure==measure)],df$one[which(df$measure==measure)]), na.rm = T))
    if(is_main){
      which <- which(df$measure==measure & df$f_or_w=="flat")
      title <- paste(measure, ": cor =",round(temp$cor_f,3))
      temp$min <- min(c(df$all[which(df$f_or_w=="flat" & df$measure==measure)],
                        df$one[which(df$f_or_w=="flat" & df$measure==measure)]), na.rm = T)
      temp$max <- max(c(df$all[which(df$f_or_w=="flat" & df$measure==measure)],
                        df$one[which(df$f_or_w=="flat" & df$measure==measure)]), na.rm = T)
    } else { 
      which <- which(df$measure==measure)
      title <- paste(measure, ": cor_f =",round(temp$cor_f,3), ": cor_w =", round(temp$cor_w,3))
    }
    ggl[[measure]] <- ggplot(df[which,], aes(x=all, y=one))+
      geom_point(size=0.1)+
      ggtitle(title)+
      scale_x_continuous(limits = c(temp$min,temp$max),
                         breaks=seq(round(temp$min),round(temp$max),by=ifelse(measure=="Zscore",1,0.1)),
                         expand = c(0,0))+
