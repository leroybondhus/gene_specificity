...
specificity_measures <- list(func_names=c("Zscore", "Tau", "Tsi","Gini"),
                             ...
                             out_type=list(Zscore="matrix",
                                           ...
                                           Tsi="vector",
                                           Gini="vector"),
                             cutpoint=list(Zscore=2,
                                           Tau=0.75,
                                           Tsi=0.75,
                                           Gini=0.75),
                             delta_cutpoint=list(Zscore=1,
                                                 Tau=0.05,
                                                 Tsi=0.05,
                                                 Gini=0.05)
                             )
...
for(is_main in c(T,F)){
  ...
  if(is_main){
    ...
    g <-arrangeGrob(grobs=grobl, ncol=4, widths=c(4,4,4,4))
    filename = paste(figures_dir, "/main_one_v_all.png", sep="")
    ggsave(filename, g, device = "png", width = 10, height = 2.8, dpi = 300)
  } else {
    grid.arrange(grobs=grobl, nrow=4, heights=c(5,3,3,3))
    g <-arrangeGrob(grobs=grobl, nrow=4, heights=c(4,4,4,4))
    filename = paste(figures_dir, "/supp_one_v_all.png", sep="")
    ggsave(filename, g, device = "png", width = 4, height = 10, dpi = 300)
  }
}

```

```{r Figure 1DE or Supp}
...
genes <- getBM(attributes=c('hgnc_symbol', 'ensembl_gene_id','go_id','name_1006'),
                 mart = ensembl, useCache = F)
genes_go_sub <- genes[grep("neuro|axon|brain|glia|oligodend", genes$name_1006),]

## perform GO analysis on difference between one v all 
## make summary GO plots

## Note: don't want to go directly from GO analysis to small set of GO id as some terms
##       may include many genes if they are overly general (e.g. if enriched term is "chemical signalling")
## 
...
for(measure in specificity_measures$func_names){
  ...
  which_go_rev <- intersect(which, which(is.element(str_split_fixed(df_1b$Var1,"[.]",2)[,1], genes_go_sub$ensembl_gene_id )))
  if(measure=="Zscore"){
    which_go_rev <- intersect(which_go_rev, which(grepl("Brain", df_1b$Var2)))
  }
  ...
  ggl[[measure]] <- ggplot(df_1b[setdiff(which,which_go_rev),], aes(x=all, y=one))+
    geom_point(size=0.1)+
    geom_point(data=df_1b[which_go_rev,], aes( x=all, y=one), color="red")+
    ggtitle(title)+
    scale_x_continuous(limits = c(temp$min,temp$max),
                       breaks=seq(round(temp$min),round(temp$max),by=ifelse(measure=="Zscore",2,0.1)),
                       expand = c(0,0))+
  ...
  ggl[[paste(measure,"_delta")]] <- ggplot(df_1b[setdiff(which,which_go_rev),], aes(x=all, y=all-one))+
    geom_point(size=0.1)+
    geom_point(data=df_1b[which_go_rev,], aes( x=all, y=all-one), color="red")+
    ggtitle(title)+
    geom_abline(slope=0,intercept=0, color="red", size=0.3)+
    theme_bw()+
      theme(panel.grid.minor.x = element_blank())
  
    print(ggl[[paste(measure,"_delta")]])
    
    print("brain var")
    print(var(df_1b[which_go_rev,]$all-df_1b[which_go_rev,]$one, na.rm = T))
    
    print("non-brain var")
    print(var(df_1b[setdiff(which,which_go_rev),]$all-df_1b[setdiff(which,which_go_rev),]$one, na.rm = T ))
    ggl[[paste(measure,"_delta_hist")]] <- ggplot(df_1b[setdiff(which,which_go_rev),], aes(x=all-one))+
    geom_density(size=0.1)+
    geom_density(data=df_1b[which_go_rev,], aes( x=all-one), color="red",alpha=0.5)+
    ggtitle(title)+
    theme_bw()+
      theme(panel.grid.minor.x = element_blank())
    print(ggl[[paste(measure,"_delta_hist")]])
...
if(is_main){
  grid.arrange(grobs=grobl, ncol=4, widths=c(5,3,3,3))
  g <-arrangeGrob(grobs=grobl, ncol=4, widths=c(4,4,4,4))
  filename = paste(figures_dir, "/main_one_v_all_test.png", sep="")
  ggsave(filename, g, device = "png", width = 10, height = 2.8, dpi = 300)
} else {
  grid.arrange(grobs=grobl, nrow=4, heights=c(5,3,3,3))
  g <-arrangeGrob(grobs=grobl, nrow=4, heights=c(4,4,4,4))
  filename = paste(figures_dir, "/supp_one_v_all_test.png", sep="")
  ggsave(filename, g, device = "png", width = 4, height = 10, dpi = 300)
}
...
for(measure in specificity_measures$func_names){
  ...
  sigOE_genes <- slice_max(temp_df, order_by=delta, n=nrow(temp_df)/100 )
  sigOE_genes <- str_split_fixed(sigOE_genes$Var1,"[.]",2)[,1]
