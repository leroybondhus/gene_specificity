...
df <- data.frame(n=numeric(), rep=numeric(),
                 ...
                 brain=logical() )
nums = as.numeric(unique(str_split_fixed(names(robustness_test_results$P1_Zscore), "[=,]", n=4 )[,4]))
reps = as.numeric(unique(str_split_fixed(names(robustness_test_results$P1_Zscore), "[=,]", n=4 )[,2]))

for(el in names(robustness_test_results)){
  P1orP2 <- str_split_fixed(el,"_",n=2)[1]
  for(n in nums){
    for(rep in reps){
      data.frame(n=n, rep=rep, jaccard= -1 , P1orP2=P1orP2, brain= -1)
    }
