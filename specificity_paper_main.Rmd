...
## do foreach over each element of permuation matrix lol so clever ## use arrayInd to get row/col given perm number
...
real_results <- foreach(perm=1:length(permutation_matrix),
                        .final = function(x) setNames(x,paste("rep=", arrayInd(1:length(permutation_matrix), dim(permutation_matrix))[,1], ",n=", arrayInd(1:length(permutation_matrix), dim(permutation_matrix ))[,2], sep = "")) )%dopar% {
  paste("rep=", arrayInd(perm, dim(permutation_matrix))[,1], ",n=", arrayInd(perm, dim(permutation_matrix ))[,2], sep = "")
}
...
real_results <- foreach(perm=1:length(permutation_matrix),
  ...
  for(measure in specificity_measures$func_names){
    ...
    #### NOTE:: MIGHT REMOVE THIS LOOP IF USING A PERMUTATION TABLE :0
    for(rep in 1:num_reps){
