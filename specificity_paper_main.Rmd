...
for(i in 1:length(spec_mats)){
  ...
  temp_list[[names(spec_mats)[i] ]] <- merge(gtex_rowinfo, cbind( Name=rownames(spec_mats[[1]]),    data.frame(signif(spec_mats[[i]],3) )) )
}

filename <- paste(figures_dir, "/specificity_scores_full_Gtex.xlsx",sep = "")
