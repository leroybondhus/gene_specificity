...
for(i in 1:length(specificity_measures$func_names)){
  el_names <- paste( c("P2_weighted_baseline_","P2_flat_baseline_", "P1_weighted_", "P1_flat_", "P2_weighted_", "P2_flat_")
                     , specificity_measures$func_names[i], sep="")
  temp_list <- setNames(list(list(), list(), list(),list(),list(),list()), nm=el_names  )
...
for(measure in specificity_measures$func_names){
  ...
  ## save baseline
  robustness_test_results[paste("P2_weighted_baseline_", measure,sep = "")] <- P2_weighted_baseline
  robustness_test_results[paste("P2_flat_baseline_", measure,sep = "")] <- P2_weighted_baseline
  
  # ## TEST that foreach loop works as expected  
  # result <- foreach(perm=1:length(permutation_matrix), .final = function(x) setNames(x,paste("rep=", arrayInd(1:length(permutation_matrix), dim(permutation_matrix))[,1], ",n=", arrayInd(1:length(permutation_matrix), dim(permutation_matrix ))[,2], sep = ""))) %dopar% {
  #   
  ...
  #   temp <- list(rep_index, sample_indices)
  # } 
    ...
    P2 <- notP1[,sample_set]; colnames(P2) <- make.unique(colnames(P2)) ## make.unique will facilitate sampling with replacement
    P1uP2 <- cbind(P1,P2)
    ...
    spec_mat_weighted <- specificity_func(P1uP2, flat)
    
    # need to fill in these with spec_score matrices for each n for each method
    # names should match 
    results <- list(
      P1_weighted_spec_score = spec_mat_weighted[,colnames(P1)],
      P1_flat_spec_score = spec_mat_flat[,colnames(P1)],
      P2_weighted_spec_score = spec_mat_weighted[,colnames(P2)],
      P2_flat_spec_score = spec_mat_flat[,colnames(P2)]
    )
    results
    }
...
for(i in 1:ncol(P2_flat_baseline)){
  plot(P2_flat_baseline[,i],P2_weighted_baseline[,i])
}
