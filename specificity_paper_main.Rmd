...
if(FALSE){  ## NOTE! This chunk can require a lot of memory and time depending if running sequential. If you want to look at robustness test results it is recommended you have enabled parallel computing (e.g. check getDoParWorkers > 3 or 4 (preferably more))
...
ggl <- list()
for(measure in unique(df$measure)){
  ...
  if(measure=="Zscore"){
    gg <- gg+facet_grid(P1orP2 ~ . , scales = "free")
  }
...
for(measure in c("Zscore", "notZscore")){
  ...
  if(measure == "Zscore"){
     gg <- gg+facet_grid(P1orP2 ~ measure , scales = "free")
     gg <- gg+theme(legend.position = "none")
  } else {
     gg <- gg+facet_wrap( ~ measure, nrow=3); plot(gg)
  }
  plot(gg)
  ggl[[measure]] <- gg
}
...
grid.arrange(grobs=grobl, ncol=2  )
...
png(filename = "heatmap.png", width=8, height=16, units="in", res=300)
heatmaps$delta
dev.off()

png(filename = "heatmap_f_w.png", width=16, height=16, units="in", res=300)
...
#"NEUROD1","NEUROD2" ,"NEUROD4", "NEUROD6","MBP", "MAP2","GFAP" ,
specific_genes <- c("SOX2",  "TPPP", "OLIG1", "PRSS1", "CTRB2", "MYH6", "MYL4")
...
png("marker_genes_grad.png", width = 10, height = 8, units = "in", res=300 )
draw(heatmaps)
dev.off()
...
dotplot(ego, showCategory=100)#top 20 
dev.off()
