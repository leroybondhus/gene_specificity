...
df_vp <- df_robustness_test_results
df_summary_by_partition <- aggregate(df_vp$variance,
                                     by=list(measure=df_vp$measure,
                                             P1orP2=df_vp$P1orP2,
                                             weight_or_flat= df_vp$weight_or_flat,
                                             n=df_vp$n),
                                     FUN=function(x) mean(x,na.rm=TRUE))
names(df_summary_by_partition)[ncol(df_summary_by_partition)] <- "mean_var"
df_summary_by_partition$sd_of_var <- aggregate(df_vp$variance,
                                     by=list(measure=df_vp$measure,
                                             P1orP2=df_vp$P1orP2,
                                             weight_or_flat= df_vp$weight_or_flat,
                                             n=df_vp$n),
...
filename <- paste(figures_dir,"/",date,"_variance_non-random_v02.png",sep = "")
...
g <- arrangeGrob(grobs =grobl, ncol=2, widths=c(5,4) )
filename <- paste(figures_dir,"/",date,"_jaccard_non-random_v03.png",sep = "")
...
for(onto in c("BP","MF","CC")){
  ...
  filename <- paste(figures_dir,"/",date,"_go_clus_",onto,"_",n,".png",sep = "")
  print(filename)
