...
for(measure in specificity_measures$func_names){
    ...
    spec_one_all_list[[paste(measure, "weighted_all",sep="_")]] <- calc_weighted_zscore_matrix(exp_mat, weights)

}


df_f <- reshape2::melt(spec_one_all_list[["Zscore_flat_all"]][,which], value.name = "all")
temp_df <- reshape2::melt(spec_one_all_list[["Zscore_flat_one"]], value.name = "one")
df_f <- merge(df_f,temp_df); df_f$f_or_w <- "flat"
df_w <- reshape2::melt(spec_one_all_list[["Zscore_weighted_all"]][,which], value.name = "all")
temp_df <- reshape2::melt(spec_one_all_list[["Zscore_weighted_one"]], value.name = "one")
...
ggl <- list()
ggl[["Zscore"]] <- ggplot(df[which(df$f_or_w=="flat"),], aes(x=all, y=one))+
  geom_point(size=0.1)+
