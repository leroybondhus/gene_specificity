...
for(el in names(robustness_test_results)){
  ...
  if(measure == "Zscore"){cuts <- seq(-4,4,0.5)   ## set cutoffs
...
ggplot(df_summary_by_brain, aes(x=cut_point, y=jaccard_mean, ymin=jaccard_mean-jaccard_sd/num_reps,ymax=jaccard_mean+jaccard_sd/num_reps, group=n, fill=n))+
  geom_ribbon(alpha=.3)+
   geom_line(aes(color=n))+
  facet_wrap(brain ~ weighted)
```
