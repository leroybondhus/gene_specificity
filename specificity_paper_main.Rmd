...
if(TRUE){
  ...
  col_funs <- list()
  col_funs[["Zscore"]] <-colorRamp2(c(-4, -2 ,0, 2, 4), c("black", "blue", "white", "yellow", "red"))
  col_funs[["Zscore"]] <-colorRamp2(c(-2, -1 ,0, 1, 2), c("black", "blue", "white", "yellow", "red"))
  for(measure in specificity_measures$func_names[which(specificity_measures$out_type=="vector")]){
    col_funs[[measure]] <-colorRamp2(c(0, 0.2, 0.5, 0.8, 1), c("black", "blue", "white", "yellow", "red"))
  }
  ...
  for( ud in c("up", "down")){
    measure <- "Zscore"
    gene_vec <- gene_list[[ud]][[measure]]
    spec_mat_name <- names(spec_mats)[intersect(grep(measure, names(spec_mats)), grep("delta", names(spec_mats)))]
    spec_mat <-  spec_mats[[spec_mat_name ]]
    temp_name <- paste(ud, spec_mat_name, sep="_")
    #heatmaps[[temp_name]] <- Heatmap(spec_mat[which,], name=temp_name, col = col_funs[[measure]])
    h1 <- Heatmap(spec_mat[gene_vec[,1],], name=temp_name, row_order = gene_vec[,1], column_order = colnames(spec_mat), col = col_funs[[measure]])
    if(length(heatmaps)==0){
      heatmaps <- h1
    } else {
      heatmaps <- heatmaps %v% h1
    }
