...
df_ext <- data.frame(n=numeric(), rep=numeric(), cut_point=numeric(),
...
for(el in names(robustness_test_results)){
 ...
 # for(i in 1:length(robustness_test_results[[el]]) ){   ### foreach at this level if implementing foreach
  ...
  df_ext <- rbind(df_ext, df_temp)
}
Sys.time()

for(samp in unique(df$samp)){
  gg<-ggplot(df[which(df$samp==samp),], aes(x=cut_point, y=jaccard, color=n, group=n))+
    geom_line()+
    facet_wrap( ~ weighted)
 plot(gg)
}
