...
specific_genes <- c("SOX1","SOX2", "NEUROD1","NEUROD2" ,"NEUROD4", "NEUROD6", "PRSS1", "MYH6", "MYH7")
...
for(measure in unique(str_split_fixed(names(spec_mats), "_", 2)[,2] )){
  if(measure=="Zscore"){
    ...
    col_funs[[measure]] <-colorRamp2(c( 0, .25, 0.5, 1),
                                     c("white", "#FFB8B8",  "red","red4"))
  }
}
...
for(measure in unique(str_split_fixed(names(spec_mats), "_", 2)[,2] )){
  ...
  if(measure=="Zscore"){
    h1 <- Heatmap(temp_mat,
      ...
      h1 <- Heatmap(temp_mat, name=temp_name,
                    cluster_rows = FALSE, show_row_names = TRUE,
                    col = col_funs[[measure]])
      }
