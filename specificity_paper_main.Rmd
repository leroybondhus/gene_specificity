...
df <- reshape2::melt(spec_mat_flat_all[,which], value.name = "flat_all")
temp_df <- reshape2::melt(spec_mat_flat_one, value.name = "flat_one")
df <- merge(df,temp_df)
temp_df
#[sample(1:nrow(df),1e4),]
ggplot(df, aes(x=flat_all, y=flat_one))+
  geom_point(size=0.1)
cor(df$spec_all,df$spec_one, use="complete.obs")
colnames(exp_mat[,which])[grep("Brain",colnames(exp_mat[,which]))]
