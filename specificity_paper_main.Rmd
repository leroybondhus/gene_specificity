...
cluster_func <- function(sim_mat){LMB_dend.add_weights(LMB_dend.add_dist_to_parent(as.dendrogram(hclust(as.dist(1-sim_mat), method = "single") ) ))}  
...
real_results <- foreach(n=1:num_brain_samples, .final = function(n) setNames(n,paste("n=",n,sep = ""))  ) %dopar% {
  for(measure in specificity_measures$func_names){
    ...
    for(i in 1:ncol(notP1)){
      ...
      P2_flat_baseline[,i] <- specificity_func(P1uP2, flat)
      P2_weighted_baseline[,i] <- specificity_func(P1uP2, weights)
    }
