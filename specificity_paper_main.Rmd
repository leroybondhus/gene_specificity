...
df_summary_by_brain <-aggregate(df$jaccard, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,brain=df$brain), FUN=function(x) mean(x,na.rm=TRUE) )
names(df_summary_by_brain)[length(names(df_summary_by_brain))] <- "jaccard_mean"
df_summary_by_brain$jaccard_sd <- aggregate(df$jaccard, by=list(n=df$n,cut_point=df$cut_point,P1orP2=df$P1orP2,weighted=df$weighted,brain=df$brain), FUN=function(x) sd(x,na.rm = TRUE) )$x 
...
gg <- ggplot(df_summary_by_brain, aes(x=cut_point, y=jaccard_mean, ymin=jaccard_mean-jaccard_sd/num_reps,ymax=jaccard_mean+jaccard_sd/num_reps, group=n, fill=n))+
  ggtitle("jaccard index for brain and  non-brain, weighted and flat measures")+
  geom_ribbon(alpha=.3)+
   geom_line(aes(color=n))+
  facet_wrap(brain ~ weighted, labeller=labeller(brain=c("TRUE"="brain", "FALSE"="non-brain"), weighted=c("TRUE"="weighted", "FALSE"="flat")) )
gg
ggsave( paste( figures_dir, "/", date, "_jaccard_index_brain_v_nonbrain_flat_v_weighted_v01.png",sep = "") , gg,
