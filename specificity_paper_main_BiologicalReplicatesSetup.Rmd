---
title: "specificity_paper_main_LeroyCopy"
...
library(data.table)
library(R.utils)
gtex <- fread(temp, skip=2, header = TRUE, sep="\t")  ## unzipped this is about 8Gb
...
gtex_sample_attributes <- fread(temp, header = TRUE, sep="\t")
which <- which(is.element(gtex_sample_attributes$SAMPID, colnames(gtex)))
gtex_sample_attributes <- gtex_sample_attributes[which,]
...
```{r downsample_full_gtex}
gtex_sub_indices <- c()
for(i in 1:length(unique(gtex_sample_attributes$SMTSD))){
  gtex_sub_indices <- c(gtex_sub_indices, which(gtex_sample_attributes$SMTSD == unique(gtex_sample_attributes$SMTSD)[i])[1:3])
}
gtex_sub_indices <- unique( c(gtex_sub_indices, which(gtex_sample_attributes$SMTSD == "Liver")[1:20]))
gtex_sample_attributes <- gtex_sample_attributes[gtex_sub_indices,]
gtex <- gtex[gtex_sub_indices,]
exp_mat <- exp_mat[gtex_sub_indices,]
```
