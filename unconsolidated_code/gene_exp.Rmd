...
```{r}
   #another function takes gene name finds corresponding row 
...
stree<-(add_gene_exp(sim_tree,gtex[9,]))
attributes(stree)
attributes(stree)$gene_exp
stree %>% set("leaves_pch",19) %>% set("leaves_col", (get_leaves_attr(stree,"gene_exp"))) %>% plot
get_leaves_attr(stree,"gene_exp")
get_leaves_attr(stree,"label")

#color to represent expression value 
#point size - sample weight 
#each parent mean expression value 
#expression - color-brewer 
is.null(attributes(sim_tree)$leaf)
attr(stree,"gene_exp.Liver")
