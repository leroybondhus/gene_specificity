...
pair_dist_from_dist_df <- function(dist_df, name1, name2 ){
...
## uses functions: is_descendant, pair_dist_from_dist_df
update_dist_df<-function(dist_df, new_parent_node){
  ...
  which_desc <- apply(as.array(samples), MARGIN=1, FUN=is_descendant, parent_node=new_parent_node)
...
sum_dist_of_children <- function(node){
  ...
  left_sum <- node$left_child$dist_to_parent + sum_dist_of_children(node$left_child)
  right_sum <- node$right_child$dist_to_parent + sum_dist_of_children(node$right_child)
  if(left_sum != right_sum){print("WARNING: Asymetric Node"); return(-Inf)}
  else(return(left_sum))
}


## dist_df is object passed and object that will control tree build
dist_tree <- function(dist_df=dist_df, is_rooted=F){
  samples <- unique(c(as.character(dist_df$sample1),
                      as.character(dist_df$sample2)))
  node_list <- list()
  ...
  while(nrow(dist_df) > 0){
    ...
    left_child$dist_to_parent <- min_pair$norm_distance - sum_dist_of_children(left_child); 
    right_child$dist_to_parent <- min_pair$norm_distance - sum_dist_of_children(right_child);
    ...
    dist_df <- update_dist_df(dist_df=dist_df, new_parent_node=parent_node)
  ...
  if(is_rooted){
    node_list$name <- "root"
    node_list$p4$dist_to_parent <- 1 - sum_dist_of_children(node_list$p4) ## when rooted forces domain to cover 0-1 by making dist to root 1 for all samples
    return(node_list)
  }
  parent_node$dist_to_parent <- 1 - sum_dist_of_children(parent_node)  ## distance is distance to an abstract root 
  return(parent_node)
...
distance_tree <- dist_tree(dist_df = dist_df)
