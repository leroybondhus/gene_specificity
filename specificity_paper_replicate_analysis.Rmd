...
for( i in 1:num_reps){
  ...
  for(tt in tissue_types$tissue){
    ...
    which_cols_balanced <- c(which_cols_balanced, which_cols_tt[1] )
  }
...
foreach(exp_mat_subset=exp_mat_subsets, .errorhandling = "pass", .combine = rbind) %dopar% {
  ...
  for(measure in specificity_measures$func_names){
    ...
    spec_weighted_balanced <- specificity_func(exp_mat[,which], weights)
    
    dot_sim <- similarity_func(exp_mat_subset$unbalanced)
    sim_tree <- cluster_func(dot_sim)
    weights <- get_weights(sim_tree, colnames(exp_mat_subset$unbalanced))
    spec_weighted_unbalanced <- specificity_func(exp_mat, weights)
