...
results <- data.frame(gene=character(),
                      ...
                      measure=character(),
                      weighted=logical(),
                      balanced_spec=logical(),
                      unbalanced_spec=logical(),
                      rep=numeric()
                      )

foreach(exp_mat_subset=exp_mat_subsets, .errorhandling = "pass", .combine = rbind) %dopar% {
  ...
  for(measure in specificity_measures$func_names){
    ...
    cor(as.vector(spec_flat_unbalanced),as.vector(spec_flat_balanced), use="complete")
    cor(as.vector(spec_weighted_unbalanced),as.vector(spec_weighted_balanced), use="complete")
    plot(as.vector(spec_flat_unbalanced),as.vector(spec_flat_balanced))
    plot(as.vector(spec_weighted_unbalanced),as.vector(spec_weighted_balanced))
