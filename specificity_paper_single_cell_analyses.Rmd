...
for(results_list_id in 1:length(results_list)){
  ...
  temp_list <- foreach(i=1:length(results), .errorhandling = "pass") %dopar% {
    ...
    balanced <- grepl("TRUE",str_split_fixed(names(results)[i], "__", 4)[3])
    rep <- as.numeric(str_split_fixed(str_split_fixed(names(results)[i], "__", 4)[4],"_",2)[2])
    if(balanced){return(NULL)} ## only need to look at unbalanced
    if(measure != "Zscore"){return(NULL)} ## only need to do for Zscore
    if(!weighted){return(NULL)} ## only need to do for either weighted OR flat
    result <- results[[i]]$spec_mat_subset
    ...
    temp_fwd <- list(flat=temp_gene_list,weighted=temp_gene_list,diff=temp_gene_list)
    if(measure =="Zscore"){
      n=5 ## top n
      for(i in get_leaves_attr(sim_tree, "label")){
        
        temp_fwd$flat$up[[measure]] <- rbind( temp_gene_list$up[[measure]],
