...
for(sp in names(exp_mat_raw_list) ){
  barplot(log10(colSums(exp_mat_raw_list[[sp]])+1))
  print(sum((colSums(exp_mat_raw_list[[sp]]) > 5e5 )))
  
  ## filter for cell types with at least 500k reads total
  exp_mat_raw_list[[sp]] <- exp_mat_raw_list[[sp]][,which(colSums(exp_mat_raw_list[[sp]]) > 5e5 )]
...
for(sp in names(exp_mat_list) ){
  ## set all below 
  exp_mat_list[[sp]][which(exp_mat_list[[sp]] < 5)] <- 0
