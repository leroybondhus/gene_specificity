...
for(results_list_id in 1:length(results_list)){
  ...
  for( i in 1:length(results)){
    csv_out_filename <- paste(figures_dir,"/","markergenetable_", names(results_list)[result_list_id] , i,".csv",sep="") 
  #temp_list <- foreach(i=1:length(results), .errorhandling = "pass") %dopar% {
    print(i) 
    measure <- str_split_fixed(names(results)[i], "__", 4)[1]
    weighted <- grepl("TRUE",str_split_fixed(names(results)[i], "__", 4)[2])
    ...
    filename <- paste(figures_dir,"/","markergeneplot_", names(results_list)[result_list_id] , i,".png",sep="")
    png(filename, width=ifelse(names(results_list)[result_list_id]=="drerio", 12, 8), height=7, units="in", res=250)
    draw(h1)
    dev.off()
  }
}
