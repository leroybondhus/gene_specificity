...
for(sp in names(exp_mat_list) ){
  exp_mat_list[[sp]] <- exp_mat_list[[sp]][,grep("nonspecific",colnames(exp_mat_list[[sp]]) , ignore.case = T, invert = T)]
...
#write.csv(result_plot_df_list$balanced_v_unbalanced_cor, file=paste(figures_dir,"/balanced_v_unbalanced_model_org_cor_table.csv",sep=""))
balanced_v_unbalanced_cor <- result_plot_df_list$balanced_v_unbalanced_cor
for(o in unique(balanced_v_unbalanced_cor$dataset)){
  for(m in unique(balanced_v_unbalanced_cor$measure)){
    flat_cor <- balanced_v_unbalanced_cor[which(balanced_v_unbalanced_cor$dataset==o &
                                                  balanced_v_unbalanced_cor$measure==m &
                                                  balanced_v_unbalanced_cor$weighted),]
    weight_cor <- balanced_v_unbalanced_cor[which(balanced_v_unbalanced_cor$dataset==o &
                                                    balanced_v_unbalanced_cor$measure==m &
                                                    !balanced_v_unbalanced_cor$weighted),]
    print(o);print(m)
    print(t.test(flat_cor$cor, weight_cor$cor, paired = T, alternative = "greater"))
  }
}
...
which <- which(result_plot_df_list$balanced_v_unbalanced$dataset == "mmusculus" &
                 result_plot_df_list$balanced_v_unbalanced$rep == 1)
rd_df <- result_plot_df_list$balanced_v_unbalanced[which,]
rd_df <- rd_df[complete.cases(rd_df),]
ggl <- list()
for(measure in unique(rd_df$measure)){
  which_measure <- which(rd_df$measure==measure)
  temp <- data.frame(min=min(c(rd_df$balanced_spec[which_measure],rd_df$unbalanced_spec[which_measure]) ),
                     max=max(c(rd_df$balanced_spec[which_measure],rd_df$unbalanced_spec[which_measure]) ),
                     cor_f = cor(rd_df$balanced_spec[which_measure][which(!rd_df$weighted[which_measure])],
                                 rd_df$unbalanced_spec[which_measure][which(!rd_df$weighted[which_measure])] ),
                     cor_w = cor(rd_df$balanced_spec[which_measure][which(rd_df$weighted[which_measure])],
                                 rd_df$unbalanced_spec[which_measure][which(rd_df$weighted[which_measure])]) )
  title <- paste(measure, ": cor_f =",round(temp$cor_f,3), ": cor_w =", round(temp$cor_w,3))
  ggl[[measure]] <- ggplot(rd_df[which_measure,], aes(x=balanced_spec, y=unbalanced_spec))+
    geom_point(size=0.1)+
    ggtitle(title)+
...
grid.arrange(grobs=grobl, ncol=4, widths=c(5,3,3,3))
g <-arrangeGrob(grobs=grobl, nrow=4, heights=c(4,4,4,4))
filename = paste(figures_dir, "/mmusculus_one_v_all.png", sep="")
ggsave(filename, g, device = "png", width = 4, height = 10, dpi = 250)
...
```{r heatmap plots}
temp_figures_dir <- figures_dir
figures_dir <- paste(figures_dir,"/heatmap_plots",sep="")
...
for(results_list_id in 1:length(results_list)){
  ...
  #temp_list <- foreach(i=1:length(results), .errorhandling = "pass") %dopar% {
    ...
    if(measure =="Zscore"){
      ...
      heatmaps <- list()
      heatmaps[["diff"]] <- Heatmap(temp_delta[c(temp_gene_list$up$gene,temp_gene_list$down$gene),], name="diff",
                                       row_order = c(temp_gene_list$up$gene,temp_gene_list$down$gene), show_row_names = FALSE,
                                       cluster_columns = sim_tree, column_order = colnames(temp_delta),
                                       col = col_funs$diff)
      heatmaps[["weighted"]] <- Heatmap(result[c(temp_gene_list$up$gene,temp_gene_list$down$gene),], name="weighted",
                                       row_order = c(temp_gene_list$up$gene,temp_gene_list$down$gene), show_row_names = FALSE,
                                       cluster_columns = sim_tree, column_order = colnames(result),
                                       col = col_funs$weighted_and_flat)
      heatmaps[["flat"]] <- Heatmap(result_pair[c(temp_gene_list$up$gene,temp_gene_list$down$gene),], name="flat",
                                       row_order = c(temp_gene_list$up$gene,temp_gene_list$down$gene), show_row_names = FALSE,
      ...
      filename <- paste(figures_dir,"/","heatmapplotdiff_", names(results_list)[results_list_id] , i,".png",sep="")
      png(filename = filename, width=8, height=14, units="in", res=250)
      print(heatmaps$diff)
      dev.off()
      
      filename <- paste(figures_dir,"/","heatmapplotflatweighted_", names(results_list)[results_list_id] , i,".png",sep="")
      png(filename = filename, width=16, height=14, units="in", res=250)
      print(heatmaps$flat + heatmaps$weighted)
      dev.off()
...
```{r go plots}
temp_figures_dir <- figures_dir
figures_dir <- paste(figures_dir,"/go_plots",sep="")
dir.create(figures_dir)
...
for(results_list_id in 1:length(results_list)){
  ...
  #temp_list <- foreach(i=1:length(results), .errorhandling = "pass") %dopar% {
    ...
    ## store plot modelled on 3D with GO enrichment in top diff genes
    filename <- paste(figures_dir,"/","gotermdotplot_", names(results_list)[results_list_id] , i,".png",sep="")
    png(filename, width=6, height=8, units="in", res=250)
    print(dotplot(ego, showCategory = n))
    dev.off()
    
    filename <- paste(figures_dir,"/","gotermemmaplot_", names(results_list)[results_list_id] , i,".png",sep="")
    png(filename, width=5, height=8, units="in", res=250)
...
```{r marker genes}
temp_figures_dir <- figures_dir
figures_dir <- paste(figures_dir,"/marker_genes",sep="")
dir.create(figures_dir)

for(results_list_id in 1:length(results_list)){
  if(names(results_list)[results_list_id]=="mmusculus"){
    specific_genes <- c("Slc12a1", "Nphs2", ## Kidney markers
                        "Ctsw","Asgr2", ## Myeloid lineage markers
                        "Prss1","Creg2"
                        )
    spec_genes_w_ids <- unique(genes_list$mmusculus[which(is.element(tolower(genes_list$mmusculus$external_gene_name),
                                                              tolower(specific_genes))),
                                             c("external_gene_name","ensembl_gene_id")])
  } 
  if(names(results_list)[results_list_id]=="drerio"){
    specific_genes <- c("notch1b", "nes", ## brain markers
                        ## eye lineage markers
                        #"rmi2","mcm10","mcph1","cep192","DEPDC4","rad9a","mms22l","tgfbr1b","kat6a"
                        #"pou3f2b","foxb1a","ascl1b","notch1b","pbx4","ntn1a","nes"
                        "myoz2a","cacna1sb", ## muscle markers
                        "hhex","nphs2","cryaa"
                        )
    spec_genes_w_ids <- unique(genes_list$drerio[which(is.element(tolower(genes_list$drerio$external_gene_name),
                                                              tolower(specific_genes))),
                                             c("external_gene_name","ensembl_gene_id")])
    
  }
  if(names(results_list)[results_list_id]=="hsapiens"){next;}
  spec_genes_w_ids <- spec_genes_w_ids[match(specific_genes[which(is.element(specific_genes,spec_genes_w_ids$external_gene_name))],
                                             spec_genes_w_ids$external_gene_name),]
  ...
  for( i in 1:length(results)){
    csv_out_filename <- paste(figures_dir,"/","markergenetable_", names(results_list)[results_list_id] , i,".csv",sep="") 
  #temp_list <- foreach(i=1:length(results), .errorhandling = "pass") %dopar% {
    print(i) 
    measure <- str_split_fixed(names(results)[i], "__", 4)[1]
    weighted <- grepl("TRUE",str_split_fixed(names(results)[i], "__", 4)[2])
    ...
    if(measure != "Zscore"){
      result <- results[[i]]$spec_mat_subset
      result_pair <- results[[str_replace(names(results)[i], "weighted_TRUE", "weighted_FALSE")]]$spec_mat_subset
      fwrite( data.frame(geneid=spec_genes_w_ids$ensembl_gene_id,
                         genenames=spec_genes_w_ids$external_gene_name,
                         weighted_specificity_value=result[spec_genes_w_ids$ensembl_gene_id],
                         flat_specificity_value=result_pair[spec_genes_w_ids$ensembl_gene_id]), csv_out_filename)
      next
    } ## only need to do following for Zscore
    print(i);print("hello");print(i)
    result <- results[[i]]$spec_mat_subset
    result_pair <- results[[str_replace(names(results)[i], "weighted_TRUE", "weighted_FALSE")]]$spec_mat_subset
    sim_tree <- results[[i]]$sim_tree_subset
    
    fwrite( rbind(cbind(data.frame(geneid=unique(spec_genes_w_ids$ensembl_gene_id),
                         genenames=unique(spec_genes_w_ids$external_gene_name),
                        flat_or_weighted = "weighted"),
                        result[spec_genes_w_ids$ensembl_gene_id,]),
                  cbind(data.frame(geneid=unique(spec_genes_w_ids$ensembl_gene_id),
                         genenames=unique(spec_genes_w_ids$external_gene_name),
                        flat_or_weighted = "flat"),
                        result_pair[spec_genes_w_ids$ensembl_gene_id,])),
            csv_out_filename)
    
    ## set up color functions for heatmaps
    col_funs <- list()
    col_funs[["weighted_and_flat"]] <-colorRamp2(c(min(c(-4, min(result, na.rm = T), min(result_pair, na.rm = T)) ),
                                             -3, -1.5, 0, 1.5, 3,
                                             max(c(4, max(result, na.rm = T), max(result_pair, na.rm = T)) )),
                                             c("darkblue", "blue", "#B8B8FF" ,"white", "#FFB8B8",  "red","red4"))
    ...
    filename <- paste(figures_dir,"/","markergeneplot_", names(results_list)[results_list_id] , i,".png",sep="")
    png(filename, width=ifelse(names(results_list)[results_list_id]=="drerio", 12, 8), height=8, units="in", res=250)
    draw(h1)
    dev.off()
  }
}
...
temp_list<-list()
for(i in 41:44){
  if(grepl("Zscore", names(results_list$mmusculus$spec_results)[i] )){
    temp_list[[paste("mouse_",stringr::str_replace(names(results_list$mmusculus$spec_results)[i],"__weighted_(TRUE|FALSE)__balanced_FALSE__rep_1",""),sep="")]] <-
    merge(spec_genes_w_ids, rbind(cbind( ensembl_gene_id=rownames(results_list$mmusculus$spec_results[[i]]$spec_mat_subset),
                                   data.frame(weighted_or_flat="weighted",specificity=signif(results_list$mmusculus$spec_results[[i]]$spec_mat_subset,3) )),
                                  cbind( ensembl_gene_id=rownames(results_list$mmusculus$spec_results[[i+4]]$spec_mat_subset),
                                   data.frame(weighted_or_flat="flat",specificity=signif(results_list$mmusculus$spec_results[[i+4]]$spec_mat_subset,3) ))) )
  } else {
    temp_list[[paste("mouse_",stringr::str_replace(names(results_list$mmusculus$spec_results)[i],"__weighted_(TRUE|FALSE)__balanced_FALSE__rep_1",""),sep="")]] <-
    merge(spec_genes_w_ids, rbind(cbind( ensembl_gene_id=names(results_list$mmusculus$spec_results[[i]]$spec_mat_subset),
                                   weighted_or_flat="weighted",data.frame(specificity=signif(results_list$mmusculus$spec_results[[i]]$spec_mat_subset,3) )),
                                  cbind( ensembl_gene_id=names(results_list$mmusculus$spec_results[[i+4]]$spec_mat_subset),
                                   weighted_or_flat="flat",data.frame(specificity=signif(results_list$mmusculus$spec_results[[i+4]]$spec_mat_subset,3) ))))
  }
  
}

filename <- paste(figures_dir, "/specificity_scores_full_mouse.xlsx",sep = "")
