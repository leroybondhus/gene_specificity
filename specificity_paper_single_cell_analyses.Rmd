...
exp_mat <- foreach(f=files, .combine = "+") %dopar% {
  exp_mat <- matrix(0, nrow=length(rownames_genes), ncol=length(unique(anno$Celltype_comb)) )
  colnames(exp_mat) <- unique(anno$Celltype_comb)
  rownames(exp_mat) <- rownames_genes
  raw_exp <- fread(f)
  rownames(raw_exp) <- raw_exp$V1
  raw_exp$V1 <- NULL
  cts <- unique(anno$Celltype_comb[which(is.element(anno$Cell_id, colnames(raw_exp)))])
  for(ct in cts){
    exp_mat[rownames(raw_exp),ct] <-  exp_mat[rownames(raw_exp),ct]+
