...
for(results_list_id in 1:length(results_list)){
  if(names(results_list)[results_list_id]=="mmusculus"){
    ...
    specific_genes <- c("Dcn","Hoxd10", "Slc12a1", "Adams16", "Pappa2", "Nphs2", "Hoxa10", "Tmem72", "Sim1",
                        "Lama1", "Hoxb9", "Six4", "Odd1"
                        # "Cebpe", "Csf1r", "Cebpa","Spi1", "Elane"," Prtn3", "Mpo", "Apoe"," Gata2", "Gata1", "Gfi1b", "Car1", "Car2", "Klf1"
                        )
  ...
  #temp_list <- foreach(i=1:length(results), .errorhandling = "pass") %dopar% {
    ...
    ## set up color functions for heatmaps
    col_funs <- list()
    col_funs[["weighted_and_flat"]] <-colorRamp2(c(min(c(-3, min(result, na.rm = T), min(result_pair, na.rm = T)) ),
                                             -2, -1.5, 0, 1.5, 2.5,
                                             max(c(3, max(result, na.rm = T), max(result_pair, na.rm = T)) )),
                                             c("darkblue", "blue", "#B8B8FF" ,"white", "#FFB8B8",  "red","red4"))
